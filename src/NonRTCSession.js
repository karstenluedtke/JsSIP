...
class DummyRTCConnection
{
  ...
  createOffer()
  {
 	 ...
 	 return new Promise((resolve, reject) => { resolve(this.dummyOffer); });
  }

  createAnswer()
  {
 	 logger.debug('DummyRTCSession.createAnswer()');
 	 return new Promise((resolve, reject) => { resolve(this.dummyAnswer); });
  }
  ...
  setLocalDescription(desc)
  {
 	 ...
 	 return new Promise((resolve, reject) => { resolve(desc); });
  }
  ...
  getStats()
  {
 	 ...
 	 const emptyStats = [ ];
 	 return new Promise((resolve, reject) => { resolve(emptyStats); });
  }
...
module.exports = class NonRTCSession extends RTCSession
{
  ...
  toString()
  {
 	 return "NonRTCSession id=" + String(this.id) +
 											", body=" + String(this.invite_body);
  }
  ...
  _sendReinvite(options = {})
  {
 	 ...
 	 function onSucceeded(response)
 	 {
 		 ...
 		 else if (!response.hasHeader('Content-Type') || this.isWrongContentType(response.getHeader('Content-Type').toLowerCase()))
 		 {
 			 onFailed.call(this);
 		 ...
 		 info.on('succeeded', (event) => {
 			 ...
 			 if (event.response?.body) {
 				 ...
 				 if (cthdr && cthdr.startsWith(contentType)) {
 					 ...
 					 logger.warn(`NonRTCSession.sendInfo: response content type mismatch: ${cthdr} != ${contentType}`);
 				 }
