...
module.exports = class Subscription extends EventEmitter
{
  ...
  scheduleRefreshTimer(seconds)
  {
    ...
    this._subscriptionEnds = Date.now() + timeout;
  ...
  receiveRequest(request)
  {
    ...
    if (request.method === JsSIP_C.NOTIFY)
    {
      ...
      if (subsState && /active/.test(subsState))
      {
        let epos = subsState.indexOf('expires=');
        if (epos > 0)
        {
          let expires = parseInt(subState.substr(epos + 8));
          let newEnd = Date.now() + 1000 * expires - 3000;
          if (newEnd < this._subscriptionEnds)
            this.scheduleRefreshTimer(expires);
        }
      }
      if (subsState && /terminated/.test(subsState))
      {
        if (this._subscriptionTimer !== null)
        {
          clearTimeout(this._subscriptionTimer);
        ...
        this._unsubscribed(request, substate)
      }
  ...
  _unsubscribed(response, cause)
  {
    ...
    this._subscribing = false;
    this._active = false;
    this.emit('ended', { originator: (response? 'remote': 'local'),
                         response: response, cause: cause });
